...
//---------------------------------------------------------------------------
int vtkMRMLVolumeRenderingDisplayableManager
::IsCurrentMapperSupported(vtkMRMLVolumeRenderingDisplayNode* vspNode)
{
...
::IsMapperSupported(vtkVolumeMapper* volumeMapper, vtkMRMLVolumeRenderingDisplayNode* vspNode)
{
  ...
  std::cout << "Input: " << volumeMapper->GetInput() << std::endl;
...
::SetupMapperFromParametersNode(vtkMRMLVolumeRenderingDisplayNode* vspNode)
{
  ...
  vtkVolumeProperty* volumeProperty = vspNode->GetVolumePropertyNode() ?
    vspNode->GetVolumePropertyNode()->GetVolumeProperty() : 0;

  if (!this->IsMapperSupported(volumeMapper, vspNode))
    {
    volumeMapper = 0;
    volumeProperty = 0;
    }
  else if (vspNode->IsA("vtkMRMLCPURayCastVolumeRenderingDisplayNode"))
    {
    ...
    this->UpdateNCIMultiVolumeRaycastMapper(vtkSlicerGPURayCastMultiVolumeMapper::SafeDownCast(volumeMapper),
                                            vtkMRMLNCIMultiVolumeRayCastVolumeRenderingDisplayNode::SafeDownCast(vspNode));
    volumeProperty = this->VolumePropertyGPURaycastII;
    }
  else if (vspNode->IsA("vtkMRMLGPUTextureMappingVolumeRenderingDisplayNode"))
    {
  ...
  std::cout << "Vsp; " << vspNode->GetClassName() << std::endl;
  std::cout << "SetMapper: " << (volumeMapper ? volumeMapper->GetClassName() : "none") << std::endl;
  std::cout << "SetMapper: " << (volumeProperty ? volumeProperty : NULL) << std::endl;
