...
vtkMRMLFiberBundleLineDisplayNode::vtkMRMLFiberBundleLineDisplayNode()
{
  ...
  this->TensorToColor->SetInputConnection(this->AssignAttribute->GetOutputPort());
...
//----------------------------------------------------------------------------
vtkPolyData* vtkMRMLFiberBundleLineDisplayNode::GetOutputPolyData()
{
  ...
  vtkPolyData *outputPolyData = NULL;
  if (diffusionTensorDisplayPropertiesNode != NULL)
    ...
    switch (this->GetColorMode())
      ...
      case vtkMRMLFiberBundleDisplayNode::colorModeScalar:
        outputPolyData = this->TensorToColor->GetOutput();
        break;
      case vtkMRMLFiberBundleDisplayNode::colorModeUseCellScalars:
        if (!this->GetInputPolyData()->GetCellData()->HasArray(this->GetActiveScalarName()) ||
          ...
          {
          outputPolyData = this->ColorLinesByOrientation->GetOutput();
          }
        else
          {
          outputPolyData = this->Superclass::GetOutputPolyData();
          }
        break;
