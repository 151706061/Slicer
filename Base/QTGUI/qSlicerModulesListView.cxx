...
void qSlicerModulesListViewPrivate::updateItem(QStandardItem* item)
{
  ...
  if (this->FactoryManager->skippedModuleNames().contains(moduleName))
    {
    item->setForeground(q->palette().color(QPalette::Disabled, QPalette::Text));
    }
  if (this->FactoryManager == 0 ||
      this->FactoryManager->modulesToSkip().contains(moduleName) )
    {
...
void qSlicerModulesListView::setFactoryManager(qSlicerAbstractModuleFactoryManager* factoryManager)
{
  ...
  if (d->FactoryManager != 0)
    ...
    disconnect(d->FactoryManager, SIGNAL(modulesToSkipChanged(QStringList)),
               this, SLOT(updateModules()));
    disconnect(d->FactoryManager, SIGNAL(moduleSkipped(QString)),
  ...
  if (d->FactoryManager != 0)
    ...
    connect(d->FactoryManager, SIGNAL(moduleInstantiated(QString)),
            this, SLOT(addModule(QString)));
    connect(d->FactoryManager, SIGNAL(modulesToSkipChanged(QStringList)),
            this, SLOT(updateModules()));
    connect(d->FactoryManager, SIGNAL(moduleSkipped(QString)),
            this, SLOT(updateModule(QString)));
...
QStringList qSlicerModulesListView::modules()const
{
  ...
  if (d->FactoryManager != 0)
    ...
    modules << d->FactoryManager->modulesToSkip();
    modules << d->FactoryManager->skippedModuleNames();
    }
...
void qSlicerModulesListView::onItemChanged(QStandardItem* item)
{
  ...
  if (item->checkState() == Qt::Checked)
    {
    d->FactoryManager->removeModuleToSkip(moduleName);
    // ensure dependencies are checked
    if (module)
      {
      foreach(const QString& dependency, module->dependencies())
        {
        d->FactoryManager->removeModuleToSkip(dependency);
        }
      }
    }
  else
    {
    d->FactoryManager->addModuleToSkip(moduleName);
    ...
    if (module)
      ...
      foreach(const QString& dependentModule, moduleManager->dependentModules(moduleName))
        {
        d->FactoryManager->addModuleToSkip(dependentModule);
        }
      }
    }
}
