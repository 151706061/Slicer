...
void qSlicerSettingsModulesPanelPrivate::init()
{
  ...
  q->registerProperty("Modules/DisableModules", factoryManager,
                      "modulesToSkip", SIGNAL(modulesToSkipChanged(QStringList)));
  ...
  // Connect Modules to skip
  QObject::connect(factoryManager, SIGNAL(modulesToSkipChanged(QStringList)),
                   q, SLOT(onModulesToSkipChanged()));
...
void qSlicerSettingsModulesPanel::restoreDefaultSettings()
{
  ...
  if (this->defaultPropertyValue("Modules/AdditionalPaths").toStringList()
      ...
      this->defaultPropertyValue("Modules/DisableModules").toStringList()
      != this->previousPropertyValue("Modules/DisableModules").toStringList())
    {
    shouldRestart = true;
    }
...
// --------------------------------------------------------------------------
void qSlicerSettingsModulesPanel::onModulesToSkipChanged()
{
  this->setRestartRequested(true);
}
