...
public:
  ...
  QStringList ModulesToSkip;
  QMap<QString, QFileInfo> SkippedModules;
...
//-----------------------------------------------------------------------------
void qSlicerAbstractModuleFactoryManager::setModulesToSkip(const QStringList& moduleNames)
{
  Q_D(qSlicerAbstractModuleFactoryManager);
  if (d->ModulesToSkip == moduleNames)
    {
    return;
    }
  qDebug( ) << "setModulesToSkip " << moduleNames;
  d->ModulesToSkip = moduleNames;
  emit modulesToSkipChanged(moduleNames);
}

//-----------------------------------------------------------------------------
QStringList qSlicerAbstractModuleFactoryManager::modulesToSkip()const
{
  Q_D(const qSlicerAbstractModuleFactoryManager);
  return d->ModulesToSkip;
}

//-----------------------------------------------------------------------------
QStringList qSlicerAbstractModuleFactoryManager::skippedModuleNames()const
{
  Q_D(const qSlicerAbstractModuleFactoryManager);
  return d->SkippedModules.keys();
}
...
void qSlicerAbstractModuleFactoryManager::registerModule(const QFileInfo& file)
{
  ...
  if (d->ModulesToSkip.contains(moduleName))
    {
    //qDebug() << "Skipped module" << moduleName;
    d->SkippedModules[moduleName] = file;
